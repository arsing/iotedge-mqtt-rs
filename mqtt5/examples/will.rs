...
fn main() {
	env_logger::Builder::from_env(env_logger::Env::new().filter_or("MQTT3_LOG", "mqtt5=debug,mqtt5::logging=trace,will=info")).init();
	...
	let will = mqtt5::proto::Publication {
	...
	runtime.spawn(
		...
		.subscribe(mqtt5::proto::SubscribeTo {
			topic_filter: topic,
			qos,
		})
	...
	let f = client.for_each(|event| {
		if let mqtt5::Event::Publication(publication) = event {
