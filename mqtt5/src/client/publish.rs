...
impl PublishRequest {
	fn new(publication: crate::proto::Publication, ack_sender: futures::sync::oneshot::Sender<()>) -> Result<PublishRequest, PublishError> {
		...
		let encode_result = packet.encode(&mut counter).and_then(|()| crate::proto::encode_variable_byte_integer(counter.0, &mut counter));
		...
		let publication = crate::proto::Publication {
			...
			payload_format: crate::proto::PayloadFormat::Bytes,
			response_topic: None,
